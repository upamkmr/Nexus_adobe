...
class AuditOrchestrator:
    ...
    def merge(
        ...
        eng_report: Dict[str, Any],
        corrob_report: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        Merges discoverability, engagement, and (optional) corroboration reports into a
        Single Unified Audit Report. Mathematically sums summary counts and concatenates
        findings into one array.

        `corrob_report`, when provided, holds findings from the live cross-web
        corroboration spot-check (see skills/discoverability-audit/references/
        corroboration-guide.md). That step requires a WebSearch tool call per claim, which
        this script cannot make itself -- it must be run by the invoking agent, which then
        writes its findings to a JSON file (same {"findings": [...]} shape as the other
        sub-reports) and passes it via --corroboration-file. Without that flag, the unified
        report simply omits this stage rather than silently pretending to include it.
        """
        ...
        corrob_findings = (corrob_report or {}).get("findings", [])

        if self.deduplicate:
            # Optional deduplication if requested: merge identical titles
            seen_titles = {}
            merged_raw = []
            for f in (disc_findings + eng_findings + corrob_findings):
            ...
            raw_concatenated = list(disc_findings) + list(eng_findings) + list(corrob_findings)
    ...
    def run(self, corrob_report: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Runs discoverability and engagement audits, captures outputs, and merges into
        unified report. `corrob_report`, if supplied by the caller (see --corroboration-file
        in main()), is folded in too -- see merge() for why this script can't produce it itself."""
        ...
        n_corrob = len((corrob_report or {}).get("findings", []))
        self._log(
            f"Captured {len(disc_report.get('findings', []))} discoverability findings, "
            f"{len(eng_report.get('findings', []))} engagement findings, "
            f"{n_corrob} corroboration findings."
        )
        if corrob_report is None:
            self._log(
                "No --corroboration-file supplied: the live cross-web corroboration "
                "spot-check (corroboration-guide.md) was not run. This report covers "
                "the static sameAs proxy check only, not the live-search stage."
            )
        unified_report = self.merge(disc_report, eng_report, corrob_report)
...
def main():
    ...
    parser.add_argument(
        "--corroboration-file",
        metavar="CORROBORATION_JSON",
        help=(
            "Optional path to a JSON file (same {\"findings\": [...]} shape as the other "
            "sub-reports) produced by following skills/discoverability-audit/references/"
            "corroboration-guide.md. That procedure needs a live WebSearch call per claim, "
            "which only the invoking agent -- not this script -- can perform. Supplying this "
            "flag is what actually includes the corroboration stage in the unified report; "
            "without it, the report is silently limited to the static sameAs proxy check."
        )
    )
    ...
    if args.from_files:
        ...
        report = orchestrator.merge(disc_report, eng_report, corrob_report)
    else:
        if not args.url:
            parser.error("A target URL or domain is required unless --from-files is provided.")
        report = orchestrator.run(corrob_report)
