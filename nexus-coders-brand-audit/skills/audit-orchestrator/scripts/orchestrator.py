...
class Orchestrator:
    ...
    def merge(self, reports: List[dict]) -> dict:
        ...
        email_kw = ('email', 'summarisation', 'summarization', 'above-the-fold', 'above-fold')

        disc_f = [f for f in raw if any(k in f['title'].lower() for k in disc_kw)]
        eng_f = [f for f in raw if any(k in f['title'].lower() for k in eng_kw)]
        email_f = [f for f in raw if any(k in f['title'].lower() for k in email_kw)]
